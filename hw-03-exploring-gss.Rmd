...
sorted_responses <- sort(response_counts)

print(sorted_responses)

```

Answer: The possible responses are *Does not apply (i do not have a job/superior/coworker)*, *Yes*, and *No*.

96 people answered *Does not apply*.

237 people answered *Yes*.

1136 people answered *No*.
...
```{r}
#| label: applicable_responses
#| eval: true

applicable_responses <- subset(gss16, !is.na(harass5) & harass5 != "Does not apply (i do not have a job/superior/co-worker)")

total_applicable <- nrow(applicable_responses)

harassed_count <- sum(applicable_responses$harass5 == "Yes")

percent_harassed <- (harassed_count / total_applicable) * 100

print(percent_harassed)

```

**Answer:**
17.26147% percent of people surveyed were harassed.
...
```{r}
#| label: email_distribution
#| eval: true

hist(gss16$email, xlab = "Minutes Spent on Email Weekly", main = "Distribution of Weekly Email Usage")

mean_email <- mean(gss16$email)
print(paste("Mean", mean_email))

median_email <- median(gss16$email)
print(paste("Median", median_email))

```

**Answer:**
Both the mean and median value for the typical amount of time responding to email is NA due to missing values in the email column.
...
```{r}
#| label: snap_insta_variable
#| eval: true

gss16 <- gss16 %>%
  mutate(snap_insta = ifelse(is.na(snapchat) & is.na(instagrm),
                              NA,
...
(*Used Chapter 4 of R for Data Science to learn Workflow: code styles, along with Chapter 13 on numbers.*)

6.  Calculate the percentage of Yes's for `snap_insta` among those who answered the question, i.e. excluding `NA`s.

```{r}
#| label: snap_insta_yes_percent
#| eval: true

total_responses <- sum(!is.na(gss16$snap_insta))

yes_responses <- sum(gss16$snap_insta == "Yes", na.rm = TRUE)

percentage_yes <- (yes_responses / total_responses) * 100
...
**Answer:**
The percentage of people who said yes in the snap_insta variable (while excluding all who said NA) was 37.46356%
...
```{r}
#| label: wrkstat_responses
#| eval: true

wrkstat_counts <- table(gss16$wrkstat)

print(wrkstat_counts)

```

**Answer:**
The possible responses are *Keeping House (284), Other (89), Retired (574), School (76), Temp not working (57), Umployed or laid off (118), Working fulltime (1321), and Working parttime (345).*
...
**Intercept** = -229.74 (representing the estimated additional time spent on email per week if all other variables are 0)

**educ (Number of years of education)** = 29.63 (expected additional minutes of emails per week for each year of school)

**Work Status** (additional minutes for each work status)
  ...
  *Temp not working* = -73.71
  
  *Unempl. laid off* = 118.35
  
  *Working fulltime* = 366.84
  
  *Working partime* = 18.90

**Snapchat/Instagram Usage** = 149.96 (expected additional minutes on email per week for those who use Snapchat and Instagram)
...
```{r}
#| label: predicted_vs_residuals
#| eval: true

plot(model$fitted.values, model$residuals,
     xlab = "Predicted Values",
     ylab = "Residuals",
     main = "Predicted Values vs. Residuals Plot")

abline(h = 0, col = "red", lty = 2)

lines(lowess(model$fitted.values, model$residuals), col = "green")

```

**Answer:**
The biggest issue with this plot is that the majority of residuals do not rest near 0. This makes it a challenge to see a discernible pattern.

*Used https://www.ncl.ac.uk/webtemplate/ask-assets/external/maths-resources/statistics/regression-and-correlation/residuals.html#:~:text=Definition,yi%E2%88%92%5Eyi. to understand the graph and role of predicted values and residuals.*
...
```{r}
#| label: support_science
#| eval: true

gss16 <- gss16 %>%
  mutate(support_science = recode(advfront,
                                  `Strongly agree` = "Yes",
                                  `Agree` = "Yes",
                                  `Disagree` = "No",
...
gss16 <- gss16 %>%
  mutate(ideology = recode(polviews,
...
```{r}
#| label: science_vs_ideology
#| eval: true

ggplot(gss16, aes(x = ideology, fill = support_science)) +
  geom_bar(position = "stack") +
  labs(title = "Relationship Between Ideology and Support for Science",
       x = "Political Ideology",
       y = "Count") +
  scale_fill_manual(values = c("Yes" = "blue", "No" = "red"),
                    name = "Support for Science") +
  theme_minimal() +
...
**Answer:**
Both of Conservative, Liberal, and Moderate leaning ideologies share similar support count of support for science. However, Conservative leaning ideologies have the greatest count of people who do not support science. Although both are similar, this graph shows that conservative leaning people are less likely to support science.
